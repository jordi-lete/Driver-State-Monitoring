...
class updateValues(QThread):
    def __init__(self, mainwindow):
            super(updateValues, self).__init__()
            self.mainwindow = mainwindow
            self.Worker = Worker()

    def calibrate(self):
        self.mainwindow.calibrateButton.setEnabled(False)
        self.mainwindow.EAR_slider.setEnabled(False)
        self.mainwindow.MAR_slider.setEnabled(False)
        self.mainwindow.FAR_lowslider.setEnabled(False)
        self.mainwindow.FAR_upslider.setEnabled(False)
        # Do stuff
        Worker.CALIBRATION_COUNTER = 0
        Worker.CALIBRATE = True

        self.mainwindow.updateButton.setEnabled(True)

    def update(self):
        print('updating')
        self.mainwindow.EAR_slider.setEnabled(True)
        self.mainwindow.MAR_slider.setEnabled(True)
        self.mainwindow.FAR_lowslider.setEnabled(True)
        self.mainwindow.FAR_upslider.setEnabled(True)
        self.mainwindow.updateButton.setEnabled(False)
        self.mainwindow.calibrateButton.setEnabled(True)

        self.mainwindow.EAR_slider.setValue(math.floor(100*EYE_AR_THRESH_NEW))
        self.mainwindow.FAR_lowslider.setValue(math.floor(100*FACE_AR_THRESH_LOWER_NEW))
        self.mainwindow.FAR_upslider.setValue(math.floor(100*FACE_AR_THRESH_UPPER_NEW))
        self.mainwindow.EAR_value.setText(str(round(EYE_AR_THRESH_NEW, 2)))
        self.mainwindow.FAR_lowvalue.setText(str(round(FACE_AR_THRESH_LOWER_NEW, 2)))
        self.mainwindow.FAR_upvalue.setText(str(round(FACE_AR_THRESH_UPPER_NEW, 2)))
...
class Worker(QThread):
    ...
    def run(self):
        ...
        # Video stream using opencv package. In this case we input the laptop webcamp (0)
        ...
        self.FIRST_FAR_LOWER_IT = True
        self.FIRST_FAR_UPPER_IT = True
        ...
        self.interval = 200
        self.Base_EAR = 0
        self.Base_FAR_LOWER = 0
        self.Base_FAR_UPPER = 0
        ...
        while self.ThreadActive:
            ...
            if Worker.CALIBRATE:
                Worker.CALIBRATION_COUNTER += 1
            # If no issues reading the frame
            if ret:
                # Convert to RGB as this is the only format that mediapipe faceMesh can process and Opencv uses BGR format
                ...
                if self.results.multi_face_landmarks:
                    ...
                    # We are calibrating
                        if Worker.CALIBRATION_COUNTER <= self.interval:
                            ...
                            global EYE_AR_THRESH_NEW
                            EYE_AR_THRESH_NEW = 0.8*self.Base_EAR
                            print(EYE_AR_THRESH_NEW)
                            self.FIRST_EAR_IT = False
                        
                        if self.interval < Worker.CALIBRATION_COUNTER <= (2*self.interval)+50:
                            cv2.putText(frame, "Turn your head 45 degrees to the left", (10,30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,0,255),2)
                            if Worker.CALIBRATION_COUNTER > (self.interval)+50:
                                self.Base_FAR_UPPER += FAR
                        elif Worker.CALIBRATION_COUNTER > (2*self.interval)+50 and self.FIRST_FAR_UPPER_IT:
                            self.Base_FAR_UPPER = self.Base_FAR_UPPER/self.interval
                            global FACE_AR_THRESH_UPPER_NEW
                            FACE_AR_THRESH_UPPER_NEW = 0.95*self.Base_FAR_UPPER
                            print(FACE_AR_THRESH_UPPER_NEW)
                            self.FIRST_FAR_UPPER_IT = False

                        if (2*self.interval)+50 < Worker.CALIBRATION_COUNTER <= (3*self.interval)+100:
                            cv2.putText(frame, "Tilt your head down", (10,30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,0,255),2)
                            if Worker.CALIBRATION_COUNTER > (2*self.interval)+100:
                                self.Base_FAR_LOWER += FAR
                        elif Worker.CALIBRATION_COUNTER > (3*self.interval)+100 and self.FIRST_FAR_LOWER_IT:
                            self.Base_FAR_LOWER = self.Base_FAR_LOWER/self.interval
                            global FACE_AR_THRESH_LOWER_NEW
                            FACE_AR_THRESH_LOWER_NEW = 1.05*self.Base_FAR_LOWER
                            print(FACE_AR_THRESH_LOWER_NEW)
                            self.FIRST_FAR_LOWER_IT = False

                        if Worker.CALIBRATION_COUNTER > (3*self.interval)+150:
                            Worker.CALIBRATE = False
                            self.FIRST_EAR_IT = True
                            self.FIRST_FAR_UPPER_IT = True
                            self.FIRST_FAR_LOWER_IT = True
